  ...
  class Cloud
    ...
    def attach_disk(vm_cid, raw_director_disk_cid)
      ...
      with_thread_name("attach_disk(#{vm_cid}, #{raw_director_disk_cid})") do
        ...
        # performed unambigously. This is typically set as part of a VM extension. Using the relative device unit
        ...
        #
        # As to why do we need to ask the vm whether disk UUIDs are enabled? Well we don't have access to the
        # vm cloud properties at disk attach time, so what we do is smuggle that information into vm's extraConfig
        # and we read it back out here so that we know whether or not it was enabled at vm creation.
